...
import { getPackagePage, updatePackagePage } from "@/actions/cms-actions";

const TABS = [
    { id: "hero", label: "Hero", icon: "home" },
    { id: "packages", label: "Package List", icon: "school" },
    ...
    return (
        <CMSPage
                ...
                if (data.categories) {
                    data.categories = data.categories.map((cat: any) => ({
                        ...
                        Packages: cat.Packages ? {
                            ...cat.Packages,
                            items: cat.Packages.items?.map((item: any) => ({
                                ...item,
            ...
            renderTabContent={(activeTab: string, data: any, setData: (d: any) => void) => {
                ...
                if (index === -1) {
                    return (
                        <div className="p-12 text-center bg-white rounded-xl border border-dashed">
                            ...
                            <Link href="/dashboard/packages/cms" className="text-blue-600 font-bold hover:text-blue-700 transition-colors">
                                Return to listings
                            </Link>
                        </div>
                    );
                ...
                const updateConfig = (updates: Partial<IPackageConfig>) => {
                ...
                return (
                    <div className="space-y-8">
                        ...
                        {activeTab === "packages" && (
                            <CMSSection title="Package Offerings">
                                <div className="space-y-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                value={config.Packages?.title || ""}
                                                onChange={(e) => {
                                                    const currentPackages = config.Packages || { title: "", subtitle: "", items: [] };
                                                    updateConfig({ Packages: { ...currentPackages, title: e.target.value } });
                                                }}
                                                className={inputClass}
                                            />
                                        </div>
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                value={config.Packages?.subtitle || ""}
                                                onChange={(e) => {
                                                    const currentPackages = config.Packages || { title: "", subtitle: "", items: [] };
                                                    updateConfig({ Packages: { ...currentPackages, subtitle: e.target.value } });
                                                }}
                                                className={inputClass}
                                            />
                                        </div>
                                    ...
                                    <div className="space-y-4 pt-4 border-t border-slate-100">
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    updateConfig({
                                                        Packages: {
                                                            ...currentPackages,
                                                            items: [...(currentPackages.items || []), { level: "", slug: "", type: "", description: "", features: [], icon: "school", popular: false }]
                                                        }
                                                    });
                                                }}
                                        ...
                                        {(config.Packages?.items || []).map((item: any, idx: number) => (
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        updateConfig({
                                                            Packages: {
                                                                ...currentPackages,
                                                                items: currentPackages.items.filter((_: any, i: number) => i !== idx)
                                                            }
                                                        });
                                                    }}
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                                    <div>
                                                        ...
                                                        <input
                                                            value={item.level}
                                                            onChange={(e) => {
                                                                const items = [...(config.Packages?.items || [])];
                                                                items[idx] = { ...items[idx], level: e.target.value };
                                                                updateConfig({ Packages: { ...config.Packages, items } });
                                                            }}
                                                            className={inputClass}
                                                    ...
                                                    <div>
                                                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Package Type Tag</label>
                                                        <input
                                                            value={item.type}
                                                            onChange={(e) => {
                                                                const items = [...(config.Packages?.items || [])];
                        ...
                        {activeTab === "content" && (
                            <CMSSection title="Page Narrative Content">
                                <RichTextEditor
                                    ...
                                    placeholder="Write main Package details here..."
                                />
                            </CMSSection>
                        )}
                                ...
                                <CMSSection title="Danger Zone">
                                    <div className="p-8 bg-rose-50 rounded-4xl border border-rose-100">
                                        ...
                                        <p className="text-rose-600 font-medium mb-8 text-sm">
                                            This will permanently delete specifically designed content for this Package slug. This action cannot be undone.
                                        </p>
                                        <button
                                            type="button"
                                            onClick={async () => {
                                                ...
                                                try {
                                                    await updatePackagePage(updated);
                                                    toast.success("Destroyed", { id: loadingToast });
                                                    window.location.href = "/dashboard/packages/cms";
                                                } catch (error) {
