...
interface IPackagePageData {
    categories: IPackageConfig[];
}

export default function PackageCMSListing() {
    ...
    const handleAddConfiguration = async (target: { name: string, slug: string, type: string }, data: IPackagePageData, setData: (data: IPackagePageData) => void) => {
        const newConfig: IPackageConfig = {
            name: target.name,
            slug: target.slug,
            ...
            hero: {
                ...
                title: target.name,
                description: `Explore our specialized ${target.name} Packages and programs.`,
                backgroundImage: "",
        ...
        try {
            await updatePackagePage(updatedData);
            setData(updatedData);
            toast.success("Ready to edit!", { id: loadingToast });
            router.push(`/dashboard/packages/cms/${target.slug}`);
        } catch (error) {
            toast.error("Failed to initialize", { id: loadingToast });
        }
    };

    const handleDelete = async (targetSlug: string, targetName: string, data: IPackagePageData, setData: (data: IPackagePageData) => void) => {
    ...
    return (
        <CMSPage
            title="Package CMS"
            description="Manage landing pages for Package categories and subcategories."
            getAction={getPackagePage}
            updateAction={updatePackagePage}
        >
            {(data: IPackagePageData, setData: (data: IPackagePageData) => void) => {
                ...
                const renderRow = (target: { name: string, slug: string, type: string }, isSub = false) => {
                    ...
                    const frontendLink = target.slug === 'default' ? '/packages' : `/packages/${target.slug}`;
                            ...
                            <td className="px-6 py-4 text-right">
                                <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    {config ? (
                                        <>
                                            <Link
                                                href={`/dashboard/packages/cms/${config.slug}`}
                        ...
                        <div className="bg-white rounded-3xl border border-slate-200/60 shadow-sm overflow-hidden">
                            <table className="min-w-full">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {renderRow({ name: "Global Package Hub", slug: "default", type: "default" })}
                                    {categories.map(cat => (
