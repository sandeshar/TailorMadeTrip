...
import { getBlogCategoryBySlug, getBlogSubcategoryBySlug } from '@/actions/categories';
...
const CombinedArticles = async ({ selectedCategory, selectedSubcategory, searchQuery, page = 1 }: { selectedCategory?: string; selectedSubcategory?: string; searchQuery?: string; page?: number }) => {
    ...
    try {
        ...
        if (selectedSubcategory) {
            const subcategoryDoc = await getBlogSubcategoryBySlug(selectedSubcategory);
            if (subcategoryDoc) {
        ...
        <section className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {articles.map((article) => (
                    <Link key={article._id} href={`/blog/${article.slug || article._id}`} className="block h-full">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className="w-6 h-6 rounded-full bg-gray-300 overflow-hidden flex items-center justify-center">
                                            {article.author ? (
                                                ...
                                                <div className="bg-primary w-full h-full flex items-center justify-center text-white text-[10px]">T</div>
                                            )}
                                        </div>
                                        <span className="text-xs font-medium text-gray-600">By {article.author || 'Travel Team'}</span>
                                    </div>
                                    <CardShare title={article.title} url={apiUrl(`/blog/${article.slug || article._id}`)} />
                                </div>
                            </div>
                        </article>
                    </Link>
